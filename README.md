...
每天到点后脚本自动执行：登录换 Cookie → 查询任务 → 等待广告时长 → 提交任务 → 领取奖励，最后推送结果通知。也可以随时在 Loon「通用脚本」里手动触发 `小米钱包手动提交` 立即执行一次（用于测试）。

### manual 模式（手动看完后确认）

原版 manual 模式是终端里按回车确认，Loon 里改成两段式：

1. cron 到点后，脚本发通知「请打开小米钱包观看视频任务广告」
2. 你在小米钱包里把广告/视频完整看完
3. 返回 Loon → 通用脚本 → 手动触发 `小米钱包手动提交`，脚本提交任务并领奖
...
- 登录换取 Cookie 依赖逐跳解析 302 响应头；若 Loon 版本对响应头处理有差异导致获取失败，请升级 Loon 并查看脚本日志（Loon → 日志）
